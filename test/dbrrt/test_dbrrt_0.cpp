...
BOOST_AUTO_TEST_CASE(test_quad3d_connect) {
  ...
  Trajectory sol;
  Result_opti result;
  trajectory_optimization(problem, traj_out, options_trajopt, sol, result);
...
BOOST_AUTO_TEST_CASE(test_uni1_bugtrap_connectorig) {
  ...
  try {
    // dbrrtConnect(problem, robot, options_dbrrt, options_trajopt, traj_out,
    //                  out_info);
    // dbrrtConnectOrig(problem, robot, options_dbrrt, options_trajopt,
    // traj_out,
    //                  out_info);
  } catch (std::exception &e) {
    std::cout << e.what() << std::endl;

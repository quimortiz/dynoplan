...
void tdbastar_epsilon(
  ...
  while (!stop_search()) {
    ...
    // {
    //   std::cout << "best node has focal heuristic = 1!, quitting!" <<
    //   std::endl; break;
    // }
    time_bench.expands++;
          ...
          for (auto &n : neighbors_n) {
            ...
            if (float tentative_g =
              ...
              if (update_valid) {
                ...
                // do the heuristic checking again for the rewiring, check a
                // single state, update its reaches goal state ?
                n->focalHeuristic = focalHeuristic;
                //  + lowLevelfocalHeuristicSingleState(solution, all_robots,
                //  n->state_eig,

    ...
    if (is_at_goal_no_constraints) {
      ...
      if (all_print) {
        ...
        while (n != nullptr) {
          ...
          const auto &arrival =
              n->arrivals[arrival_idx]; // get the struct, exact one using
                                        // arrival_idx
          n = arrival.came_from;        // get the parent
          if (n != nullptr) {
            std::cout << "came_from: " << n->state_eig.format(dynobench::FMT)
                      << std::endl;
          }
          arrival_idx =
              arrival.arrival_idx; // get the best_node.current_arrival_idx in
                                   // order to get the exact element of the
                                   // current node's arrivals vector.
        }
      }
